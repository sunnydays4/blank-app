...
pourcentages = {
    "Urbanisé": surface_urbanisation / total_analyse * 100,
    "Naturelle artificielle": surface_naturelle_artificielle / total_analyse * 100,
    "Naturelle existante": surface_naturelle_existante / total_analyse * 100
}

# --- Couleurs RGB associées ---
couleurs = {
    "Urbanisé": rgb_marqueur_urbanisation,
    "Naturelle artificielle": rgb_marqueur_naturelle_artificielle,
    "Naturelle existante": rgb_marqueur_naturelle_existante
}

# --- Affichage du tube horizontal ---
fig, ax = plt.subplots(figsize=(8, 2))
start = 0
for zone, pct in pourcentages.items():
    ax.barh(0, pct, left=start, color=[c / 255 for c in couleurs[zone]], edgecolor='black')
    start += pct
...
ax.set_yticks([])
ax.set_xticks([])
ax.set_title("Répartition des surfaces (hors background)", fontsize=14)

# Ajout des étiquettes au centre des segments
start = 0
for zone, pct in pourcentages.items():
    ax.text(start + pct / 2, 0, f"{zone}\n{pct}%", va='center', ha='center', fontsize=9, color='white', fontweight='bold')
    start += pct
